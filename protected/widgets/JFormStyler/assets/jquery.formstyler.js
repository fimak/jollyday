	...
	$.fn.styler = function(opt) {
		...
		return this.each(function() {
			...
			// radio
				...
				el.css({position: 'absolute', zIndex: '-5', display: 'none'}).each(function() {
			...
			// select
				...
				el.each(function() {
					if (el.next('span.jqselect').length < 1) {
						// одиночный селект
						function doSelect() {
                                                        var selectWidth = el.outerWidth() + 'px';
                                                    
							var selectbox =
								$('<span class="selectbox jqselect" style="display:inline-block;position:relative;z-index:' + opt.zIndex + ';width:' + selectWidth + '">'+
							...
							if (el.is(':disabled')) {
								...
								var dropdown =
									$('<div class="dropdown" style="position:absolute;overflow:auto;overflow-x:hidden;width:' + selectWidth + '">'+
